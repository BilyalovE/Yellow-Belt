//
//  main.cpp
//  Task 11 Построение арифметического выражения. Часть 2
//
...
deque <string> AssembleMathExpression(const int a, const vector <char>& list_operation,
    ...
    for (int i = 0; i < list_operation.size(); ++i){
        ...
        if(i != 0){
            if((list_operation[i] == '*' || list_operation[i] == '/') &&
               (list_operation[i-1] == '+' || list_operation[i-1] == '-')){
                result.push_front("(");
                result.push_back(")");
            }
        }
        // Добавляю дейстивие отворматированное " "
        result.push_back(" " + str + " ");
        // Присоединяю число к выражению с првоеркой на отрицательность
        if (list_numbers[i] < 0){
            result.push_back("(");
